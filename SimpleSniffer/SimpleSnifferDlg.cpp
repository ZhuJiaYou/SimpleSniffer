...
BOOL CSimpleSnifferDlg::OnInitDialog()
{
	...
	m_listPackage.InsertColumn(0, _T("编号"), 3, 40);  //1表示右，2表示中，3表示左
	m_listPackage.InsertColumn(1, _T("时间"), 3, 130);
	m_listPackage.InsertColumn(2, _T("长度"), 3, 72);
	m_listPackage.InsertColumn(3, _T("源MAC地址"), 3, 140);
	m_listPackage.InsertColumn(4, _T("目的MAC地址"), 3, 140);
	m_listPackage.InsertColumn(5, _T("协议"), 3, 70);
	m_listPackage.InsertColumn(6, _T("源IP地址"), 3, 145);
	m_listPackage.InsertColumn(7, _T("目的IP地址"), 3, 145);

	m_comboNetwork.AddString(_T("选择监听的网卡接口"));
...
void CSimpleSnifferDlg::OnBnClickedStartButton()
{
	...
	{
		if(MessageBox(_T("                 您确定不保存本次监听数据吗？\n点击“是”将会清除本次监听数据，点击“否”将会保存数据。"),
...
int CSimpleSnifferDlg::updateTree(int index)
{
	...
	//处理帧数据
	HTREEITEM frame = this->m_treePackage.InsertItem(_T("链路层数据"), data);
	//源MAC
	str.Format(_T("源MAC："));
	for(i = 0; i<6; i++)
	{
	...
	{
		HTREEITEM arp = this->m_treePackage.InsertItem(_T("ARP协议头"), data);
	...
	else if(0x0800 == local_data->ethh->type)  //IP
	{
		...
		HTREEITEM ip = this->m_treePackage.InsertItem(_T("IP协议头"), data);
		...
		{
			HTREEITEM icmp = this->m_treePackage.InsertItem(_T("ICMP协议头"), data);
